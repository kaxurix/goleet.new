...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="px-4 mx-auto max-w-7xl sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center flex-shrink-0 gap-2">
            ...
            />
            <span
              className={`font-extrabold text-2xl tracking-tight ${logoClass}`}
            >
              Goleet
              <span
                className={
                  isLanding && !scrolled ? "text-white/70" : "text-primary-400"
                }
              >
                .id
              </span>
          ...
          {!nonSearchPath.includes(location.pathname) && (
            <form
              onSubmit={handleSearch}
              className="flex-1 hidden max-w-md mx-6 md:flex"
            >
              <div className="relative w-full">
                ...
                <input
                  ...
                  className="w-full py-2 pl-10 pr-4 text-sm transition-all border rounded-xl border-surface-200 bg-surface-50 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white"
                />
              </div>
            </form>
          )}
          ...
          <nav className="items-center hidden gap-1 md:flex">
            ...
            <div className="w-px h-5 mx-2 bg-current opacity-20" />
      ...
      {/* Mobile Drawer */}
      <div
        className={`md:hidden bg-white border-t border-surface-100 shadow-lg transition-all duration-300 overflow-hidden ${mobileOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
      >
        <div className="px-4 py-4 space-y-1">
          ...
          <div className="pt-2 border-t border-surface-100">
            {isLoggedIn ? (
              ...
              <Link
                to="/auth"
                className="flex items-center justify-center w-full gap-2 py-3 text-sm font-semibold text-white transition-all rounded-xl bg-primary-600 hover:bg-primary-700"
              >
                <LogIn className="w-4 h-4" />
                Masuk / Daftar
              </Link>
